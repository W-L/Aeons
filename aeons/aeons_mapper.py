...
class LinearMapper:
    ...
    def __init__(self, ref: str, mu: int = 450, workers: int = 8, default: bool = True):
        """
        Initialize a LinearMapper object; wrapper for minimap2's mappy implementation
        For the default case of mapping against some linear references

        :param ref: The path to the reference FASTA file.
        :param mu: Length of anchor bases for simulations, defaults to 450.
    ...
    def mappy_batch(self, sequences: dict[str, str], out: str = None, truncate: bool = False, log: bool = True) -> str:
        """
        Map a full batch of reads and return the PAF formatted mapping hits.
        Can also perform simulated mappings of truncated (mu-length) reads.

        :param sequences: A dictionary of read_id: sequence items.
        :param out: The name of the file to output mappings, defaults to None.
        :param truncate: Whether to truncate the input reads for in silico experiments, defaults to False.
    ...
    def map_query(self, query: Tuple[str, str]) -> List[str]:
        """
